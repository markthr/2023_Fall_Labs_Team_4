...
void cardioid(float t, float a, float& x, float& y) {
  x = a * (1+ cos(t) + cos(2*t));
  y = a * (1 + sin(t) + cos(2*t));
}
...
void loop() {
  ...
  cardioid(0.0, leminscate_a, x0, y0);

  float last_x, last_y;
  // leminscate_of_bernoulli(-leminscate_t_scale * target_period_ms / 1000.0, leminscate_a, last_x, last_y);
  cardioid(-leminscate_t_scale * target_period_ms / 1000.0, leminscate_a, last_x, last_y);
  ...
  while (true) {
    ...
    cardioid(leminscate_t_scale * t, leminscate_a, x, y);
